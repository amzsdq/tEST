...
def decide(elapsed_server_seconds,target_seconds=900,stretch_seconds=1200,qualifying_work=True,finalization_safe=True):
 ...
 for name,v in [('qualifying_work',qualifying_work),('finalization_safe',finalization_safe)]:
  if not isinstance(v,bool):raise ValueError('invalid '+name)
 if target_seconds<=0 or stretch_seconds<target_seconds:raise ValueError('invalid target/stretch relationship')
 if elapsed_server_seconds>=target_seconds and not finalization_safe:
  return {'state':'FINALIZATION_BLOCKED','admit_new_package':False,'reason':'target crossed but safe finalization unavailable'}
 if target_seconds<=elapsed_server_seconds<stretch_seconds and qualifying_work:
  return {'state':'CONTINUE_STRETCH','admit_new_package':True,'reason':'minimum target crossed; continue useful work toward stretch target'}
 if elapsed_server_seconds>=target_seconds:
  return {'state':'RESERVE_ENTRY','admit_new_package':False,'reason':'safe reserve entry; no useful stretch work or stretch target reached'}
