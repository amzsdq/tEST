...
def marker(m,kind):
    ...
    strict_int(m.get("id"),f"{kind} marker id",True)
...
def validate(r):
    ...
    mutations=strict_int(r["scheduler_mutation_count"],"scheduler_mutation_count")
    target=strict_int(r.get("target_seconds",900),"target_seconds",True)
    exact=int((end-start).total_seconds()) if end else None;lower=int((bound-start).total_seconds()) if not end and bound else None
    crossed=exact>=target if exact is not None else (lower>=target if lower is not None else False);final=r.get("finalization_signature",False)
...
def validate_fixture(f):
    result=validate(f["record"])
    for k,v in f["expected"].items():
        if k!="raises" and result.get(k)!=v:raise AssertionError(f"{f['id']}: {k}: expected {v!r}, got {result.get(k)!r}")
    return result
def main():
