...
def require_marker(marker: dict[str, Any] | None, kind: str) -> None:
    ...
    if not isinstance(marker, dict):
        raise ValueError(f"{kind}: marker must be object")
    marker_id = marker.get("id")
    if isinstance(marker_id, bool) or not isinstance(marker_id, int) or marker_id <= 0:
        raise ValueError(f"{kind}: invalid marker id")
    if "created_at" not in marker or not isinstance(marker["created_at"], str) or not marker["created_at"]:
        raise ValueError(f"{kind}: missing created_at")
...
def validate(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(record["invocation_id"], str) or not record["invocation_id"]:
        raise ValueError("invalid invocation_id")
    if not isinstance(record["automation_id"], str) or not record["automation_id"]:
    ...
    end = ts(record["end"]["created_at"]) if record.get("end") else None
    ...
    if isinstance(target, bool) or not isinstance(target, int) or target <= 0:
        raise ValueError("invalid target_seconds")
    ...
    elif not end and boundary and not final_sig: classification = "ABRUPT_NONFINAL"
    ...
    return {"invocation_id": record["invocation_id"], "classification": classification, "exact_duration_seconds": exact,
            "censored_lower_bound_seconds": lower, "target_seconds": target, "target_crossed": target_crossed,
            "scheduler_mutation_count": mutations, "violations": violations, "valid": not violations}
