...
class SchedulerV2Tests(unittest.TestCase):
 ...
 def test_vevent_tzid_exact_readback(self):
  ...
  self.assertTrue(verify_readback("2026-09-25T03:17:24Z",live)["exact_match"])
 def test_vevent_z_exact_readback(self):
  live="BEGIN:VEVENT\nDTSTART:20260925T032024Z\nRRULE:FREQ=HOURLY\nEND:VEVENT"
  self.assertTrue(verify_readback("2026-09-25T03:17:24Z",live)["exact_match"])
 def test_missing_dtstart_rejected(self):
  with self.assertRaisesRegex(ValueError,"DTSTART missing"):verify_readback("2026-09-25T03:17:24Z","BEGIN:VEVENT\nRRULE:FREQ=HOURLY\nEND:VEVENT")
