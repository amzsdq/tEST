...
class RecoveryTests(unittest.TestCase):
 ...
 def test_noncanonical_raw_marker_timestamps_fail_closed(self):
  for value in ('2026-09-25T00:00:00+00:00','2026-09-25T00:00:00.000Z','2026-W39-5T00:00:00Z'):
   with self.subTest(marker='start',value=value):
    r=dict(R);r['start']={'id':1,'created_at':value};self.assertEqual(decide(r,L)['action'],'INVALID_SESSION')
   with self.subTest(marker='boundary',value=value):
    r=dict(R);r['last_durable_boundary']={'id':2,'created_at':value};self.assertEqual(decide(r,L)['action'],'RECONSTRUCT_BEFORE_RESUME')
   with self.subTest(marker='end',value=value):
    r=dict(R);r['end']={'id':3,'created_at':value};r['end_verified']=True;self.assertEqual(decide(r,L)['action'],'RECONSTRUCT_BEFORE_RESUME')
