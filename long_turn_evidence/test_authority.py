...
class AuthorityTests(unittest.TestCase):
    def test_exact_940_preferred_without_rewriting_history(self):
        self.assertIsNone(LT03["end"])
        result = select_authority(LT03, EXACT)
        self.assertEqual(result["kind"], "exact")
        self.assertEqual(result["exact_duration_seconds"], 940)
        self.assertTrue(result["target_crossed"])

    def test_start_mismatch_fails_closed(self):
        bad = [dict(EXACT[0], start={"id": 999, "created_at": "2026-09-24T15:16:24Z"})]
        with self.assertRaisesRegex(ValueError, "EXACT_START_MISMATCH"):
            select_authority(LT03, bad)

    def test_automation_mismatch_fails_closed(self):
        bad = [dict(EXACT[0], automation_id="wrong")]
        with self.assertRaisesRegex(ValueError, "EXACT_AUTOMATION_MISMATCH"):
            select_authority(LT03, bad)

    def test_ambiguous_exact_fails_closed(self):
        duplicate = dict(EXACT[0], evidence_id="duplicate")
        with self.assertRaisesRegex(ValueError, "AMBIGUOUS_EXACT_AUTHORITY"):
            select_authority(LT03, EXACT + [duplicate])
