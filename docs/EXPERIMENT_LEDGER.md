...
| P4-WAKE-OMIT-01-A | P4V7 | routine positive WAKE_OK field | 3m class | 1 | YES | YES via update return | omitted = YES by scheduled invocation | YES | non-authoritative | omitted = NO observed | inherited unchanged P1/E8 path | compact Issue sample without positive WAKE_OK + next-turn reconstruction | lower | PASS | SAMPLE-1 preserved trial/order/status/NEXT; next scheduled invocation established positive wake. |
| P4-WAKE-OMIT-01-B | P4V7 | routine positive WAKE_OK field | 3m class | 1 | YES | YES via update return | omitted = YES by scheduled invocation | YES | non-authoritative | omitted = NO observed | inherited unchanged P1/E8 path | second reconstruction + boundary reconciliation | lower | PASS | SAMPLE-2 reconstructed SAMPLE-1; transient Issue-write block recovered before reconciliation. |
...
## Reconciliation note — 2026-09-22 21:27 KST

P4V7 positive WAKE_OK omission is provisionally promoted after SAMPLE-1 and SAMPLE-2 reconstruction. In routine clean-success non-boundary records, omitted WAKE_OK is unambiguous because the current scheduled invocation itself is positive wake evidence. WAKE_OK remains explicit for PENDING/NO, anomaly, recovery, rollback, boundary, and timing-specific evidence. The full historical ledger was restored from authoritative pre-compaction commit 80d91516 after the accidental compaction at the prior boundary; historical rows were preserved and the two P4V7 rows were appended.

## Reconciliation note — 2026-09-22 21:49 KST

P4V8 positive STATE_OK omission is provisionally promoted. APPLY, SAMPLE-1, and SAMPLE-2 preserve candidate/trial/sample order and four-way status semantics. In routine clean-success non-boundary records, omitted STATE_OK means the final scheduler update return object confirmed expected RRULE/DTSTART/is_enabled; omitted WAKE_OK remains YES by actual scheduled invocation. STATE_OK/WAKE_OK remain explicit for PENDING/NO, malformed/mismatch, anomaly, recovery, rollback, boundary, and timing-specific evidence. Historical rows were restored from full canonical commit 797b27e0 and preserved while appending the two P4V8 rows.

```text
CURRENT_CANDIDATE=P4V8 on P1 single-final-write path
LEAD_TIME=+3m class
...
ROUTINE_LOGGING=Issue #1 only; clean-success non-boundary schema={EXPERIMENT,SAMPLE,RESULT,WRITE_OK,WORK_OK,NEXT}; omitted STATE_OK means YES via valid scheduler update return; omitted WAKE_OK means YES by actual scheduled invocation; omitted DUPLICATE means NO duplicate/anomaly observed; START/END only for boundary/anomaly/rollback/timing evidence; non-positive/anomalous state and wake evidence remain explicit
KNOWN_FAILURES=+2m mixed reliability; dispatch timestamp anomalies; arbitrary external non-idempotent effects require destination support; full-file boundary ledger replacement can accidentally compact history if source is not merged
RECOVERY_EVIDENCE=P1-2M-01 and E8-P1-COLD-01
NEXT_DISCRIMINATING_TEST=verify restored ledger row preservation, then select one further simplification with P4V8 fixed; do not alter scheduler, +3m lead, checkpoint, authority, verification, bootstrap, or recovery semantics
```
